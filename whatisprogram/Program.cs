using System; //정규화하지 않고 바로바로 쓰겠다
...
namespace whatisprogram
{
    internal class Program
    {
        static void Main(string[] args)
        {
             ...
             * 오류를 탐색하고 수정하는 과정을 디버그(Debug), 디버깅(Debugging) 또는 트러블슈팅(Trouble shooting)
             * 이라고 한다.
             * 
             * C#의 경우
             ...
             * 프로그래밍 언어이다.
             * 
             * C#은 C, C++, Java, Javescript와 기초 문법이 비슷하다.
             * C#은 자동으로 메모리를 관리한다. (Garbage collection 기능을 제공)
             * C#은 컴파일 기반 언어이다.
             * C#은 강력한 형식(Strongly tybed)의 언어이다.
             * 
            ...
            /**
             * C#의 기본 코드 구조
             * C# 프로그램은 class와 Main() 매서드가 반드시 있어야 하고, 하나 이상의 문(Statement)이 있어야 한다.
             * C#의 기본 코드는 위쪽에 네임스페이스 선언부와 Main() 매서드가 오고, 중괄호 시작과 끝을 사용하여
             * 프로그램 범위(Scope)를 구분한다.
             * 
             * 네임스페이스: 자주 사용하는 네임스페이스를 위쪽에 미리 선언해둘 수 있다.
             * Main() 매서드: 프로그램의 시작 지점이며, 반드시 있어야 한다.
             * 중괄호 ({}) : 프로그램의 범위를 구분 짓는다.
             * 세미콜론(;): 명령어(문, 문장)의 끝을 나타낸다.
             * 
             * Main() 매서드
             * 매서드(Method): 클래스에서 제공하는 맴버 함수를 의미한다.
             * Main() 앞에 static 키워드가 있어 개체를 생성하지 않고 바로 실행할 수 있다.
             * Main() 매서드가 2개이면 "프로그램 진입점이 2개 이상 정의되어 있습니다." 라는
             * 에러 메시지가 출력되어 프로그램이 컴파일되지 않습니다.
             * 
             * 대, 소문자 구분하기
            ...
            /**
             * 이스케이프 시퀸스
             * C#은 WriteLine() 매서드에서 사용할 확장 문자를 제공하는데, 이를
             * 이스케이프 시퀸스(Escapesequence)라고 한다.
             * 역슬래시(\) 기호와 특정 문자를 조합하면 특별한 기능을 사용할 수 있다.
            ...
            string name = "임준일";
            string stu = "(게임콘텐츠제작) 모바일 게임 개발자 양성과정C";
            string worki = "(전공)게임 프로그래밍 기초 기술";

            Console.WriteLine("{0}", name);
            Console.WriteLine("{0}", stu);
            Console.WriteLine("{0}", worki);
            Console.WriteLine($"{name}, {stu}, {worki}");

            /**
             * 변수
             * 프로그램에서 값을 다루려면 데이터를 메모리에 잠시 보관해 놓고 사용할 수 있는 임시 저장공간이 필요하다.
             * 이때 변수를 사용한다. 변수를 사용하는 순서는 선언(메모리에 공간을 확보)하고 정의(대입, 할당)하여
             * 사용하는 것이다. 변수는 데이터 양식, 변수의 이름, 대입한 값으로 이루어져 있다.
             * 
             ...
             * 실수형 리터럴: 대문자 F 또는 소문자 f를 접미사를 붙여 표현한다.    ex) 3.14F
             * 문자형 리터럴: 작은 따옴표로 묶여서 표현한다.                    ex) 'A'
             * 문자열 리터럴: 큰 따옴표로 묶여서 표현한다.                      ex) "Hello"
             * 
             * 숫자 구분자 사용
            ...
            int clhalf = 3;          // 원의 반지름 선언
            const double CL = 3.14;  // 원주율을 상수 선언
            Console.WriteLine("{0}", clhalf * clhalf * CL);*/
             ...
             * 
             * null 기능 형식(Nullable)
             * 숫자 형식의 변수를 선언할 때 int?, float?와 같이 물음표(?) 기호를 붙이면 null 기능 형식으로
             * 변경된다. 이러한 null 기능 형식에는 아무런 값도 없음을 의미하는 null을 대입할 수 있다.
             */
             ...
             * 표현 방식이다. 일반적으로 열거형으로 줄여 말한다. 열거형은 enum 키워드를 사용하고 이늄 또는 이념으로
            ...
            //int.TryParse(stringNumber, out intNumber3); //가장 추천

            ////Console.WriteLine("입력한 숫자 + 10은(는) {0}입니다.", intNumber + 10);
            ////Console.WriteLine("입력한 숫자 + 10은(는) {0}입니다.", intNumber2 + 10);

            //Console.WriteLine("입력한 숫자 + 10은(는) {0}입니다.", intNumber3 + 10);

            Console.Write("구의 반지름을 입력하시오: ");
            float clchalf = Convert.ToSingle(Console.ReadLine());
            const float CL = 3.14f;
            float powofthree = clchalf * clchalf * clchalf;

            Console.WriteLine("{0}", 4 * CL * clchalf * clchalf);
            Console.WriteLine("{0}", 4 * CL * powofthree / 3);
